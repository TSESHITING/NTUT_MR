# 👁️ 透過他們的眼睛：學障兒童的 MR 共情體驗 (Through Their Eyes)
...
* **動態影片材質貼圖技術 (Video-Mapped Textures)：** 捨棄高耗能的實時 Shader 運算，改將預先製作好的 **MP4 視覺混淆影片** 結合 Unity Video Player 元件，直接映射至空間中的虛擬模型材質上。此方法能在完全不佔用頭顯渲染效能的情況下，精準且流暢地呈現文字跳動、反轉與模糊的完美實時動態視覺。
...
* **Dynamic Video-Mapped Textures:** Utilizes optimized **MP4 video textures** integrated via Unity's Video Player component onto spatial meshes, delivering perfectly synchronized and high-performance visual confusion effects (jumping, mirroring, and blurring) without heavy runtime rendering overhead.
